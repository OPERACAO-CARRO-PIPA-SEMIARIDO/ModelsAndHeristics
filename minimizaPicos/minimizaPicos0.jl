...
usoDiario = [round(i * 0.02, digits=2) for i in duas_colunas_b[2]]
capaciadeReservatorio = convert(Vector{Float64}, duas_colunas_b[1])

remover = trues(length(usoDiario))
remover[quebraCarnaval] .= false
U = usoDiario[remover]
C = capaciadeReservatorio[remover]
