        ...
        try
            ...
            for k in nd
                ...
                if hasproperty(df_ws, col_sym)
                    ...
                    if soma > max_pico
                        max_pico = round(Int, soma)
                    end
                end
            end
            if max_pico > 0
                set_start_value(y, Float64(max_pico))
            end
    ...
    for (hora, meta) in zip(horas_checkpoints, segundos_checkpoints)
        ...
        try
            ...
            if isa(e, InterruptException)
                ...
                if has_values(model)
                    salvar_minpicos(model, caminho_pasta, "$(hora)h_INT")
                end
                return
            else
        ...
        if has_values(model)
            ...
            if obj < melhor_obj_encontrado
                println(">>> Melhor solucao na hora $hora: Obj=$obj Pico=$pico")
        ...
        if termination_status(model) == MOI.OPTIMAL
            println("Otimo comprovado. Finalizando.")
            break
        end
    end
end
...
function salvar_minpicos(model, pasta, sufixo)
    ...
    nomes = Symbol.(["Beneficiarios"; string.(collect(nd))])
...
warm_start_limite = joinpath(TEST_DIR, "abastecimento_heu_limite.csv")
warm_start_full = joinpath(TEST_DIR, "abastecimento_heu_full.csv")
