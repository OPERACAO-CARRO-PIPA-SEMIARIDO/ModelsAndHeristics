...
const ABASTECIMENTO_FILE = joinpath(@__DIR__, "resultados_minpicos", "abastecimento_melhor_absoluto.csv")
const OUTPUT_ALOCACAO   = joinpath(@__DIR__, "alocacao_m2_minpicos.csv")
const OUTPUT_CUSTO      = joinpath(@__DIR__, "custos_m2_minpicos.csv")
const NUM_MANANCIAIS    = 40
...
gap_val    = try MOI.get(model, MOI.RelativeGap()) catch; 0.0 end
