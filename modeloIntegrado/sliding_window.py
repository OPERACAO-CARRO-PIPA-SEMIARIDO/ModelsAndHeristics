...
OVERLAP = 7
PESO_PICO = 0.0
OUTPUT_BASE = "resultados_sliding_45_7"
    ...
    while dia_inicio <= TOTAL_DIAS:
        ...
        try:
            ...
            if not volumes_todos.exists():
                print(f"    ERRO: O modelo não gerou volumes_todos_dias.csv. Provável infactibilidade ou erro no solver.")
                break
            ...
            if col_name in df_vol.columns:
                ...
                print(f"    Volumes para o dia {proximo_dia_inicio} extraídos com sucesso (Local: {col_name}).")
            else:
                print(f"    AVISO: Coluna local {col_name} não encontrada. Usando volumes_finais.csv como fallback.")
...
def consolidar_resultados(output_dir, num_periodos):
    ...
    if df_abast_global is not None:
        ...
        print(f"    - abastecimento_GLOBAL.csv ({len(df_abast_global.columns)-1} dias)")
        print(f"    - alocacao_GLOBAL.csv ({len(df_aloc_global.columns)-1} dias)")
