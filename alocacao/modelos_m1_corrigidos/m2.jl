...
function resolve_M2(NM, NB, ND, matriz_dist, matriz_demanda, cap_max)
    ...
    # x[i,j,k]: viagens do manancial i ao beneficiário j no dia k (apenas onde há demanda)
    ...
    # Restrição de fonte única: cada beneficiário usa exatamente 1 manancial o ano todo
    @constraint(linModel, fonte_unica[j=1:NB],
        sum(y[i,j] for i in 1:NM) == 1)

    # Amarra x a y: só pode enviar viagens pelo manancial atribuído
    @constraint(linModel, amarra_x_y[i=1:NM, j=1:NB, k=1:ND; matriz_demanda[j,k] > 0],
        x[i,j,k] <= matriz_demanda[j,k] * y[i,j])
